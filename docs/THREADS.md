...
| 19 | grok: structured output (`--json-schema` works) so it can synthesize; staggered lane starts; upgrade to the current release | IN-PROGRESS | Started 2026-08-25; upgrade and structured output landed (`supportsJsonSchema: true`, `fixtures/grok-json-schema.ndjson`), stagger still to do; ACP long-lived mode (`grok agent stdio`) rejected for now — rationale in `docs/DESIGN.md` |
