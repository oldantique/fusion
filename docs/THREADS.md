...
| 20 | Open-source release on GitHub (`oldantique/fusion`) | IN-PROGRESS | Landed: repo metadata in `package.json`, `SECURITY.md`, `CONTRIBUTING.md`, `.github/` (CI running the three checks on a runner with no CLIs installed, bug template), and a pass generalizing facts that were only true on the owner's machine. Remaining: README positioning/intro, a UI screenshot, and a release tag |
