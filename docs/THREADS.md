...
| 21 | Re-verify all four CLIs — each updated itself past the build `fixtures/README.md` vouches for | ON-HOLD | Found and deferred by the owner 2026-09-20. `npm run smoke` and `npm run canary` passed that day and a full fused turn ran, but no premise was re-tested and no row was added. `npm run check-updates -- --help-diff` shows changes in codex, kimi and grok (kimi reworded its permission modes and upstream has a new major); procedure: the `cli-upgrade-recapture` invariant in the sync-docs skill. Until then `hooks/pre-commit` refuses commits that touch lane code or fixtures |
