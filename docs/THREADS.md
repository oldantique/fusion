...
| 23 | Snapshot each lane's advertised tool list, like the `--help` baselines | OPEN | A new tool (grok's feedback one) and claude's pending MCP servers were found by eye; `--help-diff` cannot show either. Shape: one sorted name per line under `fixtures/help/`, taken from the `system` init record of one trivial capture (claude and grok: `tools`, plus claude's `mcp_servers`), and the `CodexErrorInfo` enum from codex's generated app-server schema; kimi prints no list. Costs a model call per CLI, so it belongs to `check-updates` on demand and the upgrade routine, never to `hooks/pre-commit` (offline) |
