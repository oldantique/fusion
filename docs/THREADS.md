...
| 13 | Versioned SQLite migrations | OPEN | v2; since 2026-09-20 data migrations are versioned (`PRAGMA user_version`, `migrate()` in `src/store/db.ts`); the additive `ALTER TABLE` loop beside it is what is left to fold in |
...
| 22 | Automated test for the UI's per-turn name resolution | OPEN | Names inside a turn come from `turns.models_json` (`mountTurn` in `web/app.js`); verified by hand in a headless browser (history vs live turn), no test pins it. Cheapest route: move the resolver into a small pure module that `node:test` can import, without adding a frontend test framework |
| 24 | The claude CLI adds its own context to every lane despite `--system-prompt` | WATCH | Found 2026-09-23 (the previous build did the same): an SDK preamble, the account's email address and an environment block (sandbox path, OS, model id, date) reach the model; a canary answer once quoted the email in a made-up command. No flag found that removes them; answers are shown only to the account owner, so the exposure is to the owner themselves. Re-check on every claude upgrade |
| 25 | Retake `docs/screenshot.png`: it still names the lanes Claude Opus 5 and Grok 4.6 | BLOCKED-by-quota | The codex subscription is out of usage until 2026-09-28, and a shot with a rate-limited lane misrepresents a normal turn; retake per the `screenshot-current` rule in the sync-docs skill once codex answers again |
| 16 | Share with a few friends over the owner's domain | ON-HOLD | Decided 2026-08-21 to defer; re-assessed and deferred again 2026-09-20. Path when resumed: Cloudflare Tunnel to the loopback-bound service + Access email allowlist (TLS and identity outside the app); `src/server/auth.ts` trusts the Access JWT as identity; `conversations.owner` column and per-owner filtering in `src/store/db.ts` + `src/server/main.ts`; per-user daily turn cap; cookie `secure` behind the proxy; per-IP login rate limit (today only a fixed delay in `src/server/main.ts`); rewrite the RUNBOOK security posture. Found 2026-09-20: no existing tunnel to reuse, so a named tunnel is created from scratch and its login and the DNS record need the owner in a browser; the subdomain (and whether it sits under the owner's publication brand) is undecided; the SSE ping already outlasts the proxy's idle timeout, so streaming needs no change. File access is already contained by the jail (#15); the subscription-sharing terms risk is the owner's call |

## Archive

- 2026-09-20 — #23 closed: `npm run check-updates -- --tools-diff` snapshots what each lane advertises to its model (claude and grok init record through the lane itself, aborted before an answer is generated; codex error variants from its generated schema; kimi prints none) into `fixtures/help/*.tools.txt`; on demand only, the pre-commit hook stays offline.
- 2026-09-20 — #21 closed: all four CLIs re-verified on the builds installed that day, one teammate per CLI — every premise held and no fixture changed; three lane fixes came out of it (claude `--strict-mcp-config`, grok `--disallowed-tools` for the vendor-feedback tool, codex's new rate-limit error variant) and three stale sentences about kimi and codex; rows in `fixtures/README.md`, rationale in `docs/DESIGN.md`.
